# wskr
A simple command line program for scanning a range of machines for logged on user, some registry value, some file or folder existence, whether no user is logged on, what the machines IP settings are etc.  No installation required, no configuration files, just download and run the EXE on your Windows machine.
